...
const ImageUpload = () => {
  ...
  if(imageUrl) {
    return <div className="flex items-center justify-center h-32 w-32 relative">
      ...
      <ImCancelCircle size={30} onClick={() => updateImageUrl("")} className="absolute cursor-pointer -right-2 -top-2 z-10 hover:scale-110" />
    </div>
  }

  return <UploadDropzone endpoint="workspaceImage" onClientUploadComplete={res => {
